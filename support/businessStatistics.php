<?php
	...
	//设置当前文件路径变量
	$from = $config['root']."support/businessStatistics.php";
	//实例化生成页码类
	$u = new Util();
	//判断日期的有效性
	if($u->validID($year) && $u->validID($month)){
		...
		if($year >= $dataAnalysisConfig['dataAnalysisStartYear'] && $year <= intval(date('Y')) && $month >= 1 && $month <= 12){
			...
			foreach($result as $item){
				...
				//跳转到错误信息页面
				$errorURL =  "../common/error.php?error=".urlencode('没有该月的微视频交易统计信息')."&back=".urlencode($from);
...
<title>微视频交易统计--ChinaVEC管理中心</title>
...
<body>
    ...
    <div id="container" class="clearfix">
        ...
        <div id="container_right" class="right">
            <h1 class="mytitle" id="title">微视频交易统计</h1>
            <div class="selTittle">
            	<span>选择月份：</span>
            	<select id="selectMonth">
                </select>
            </div>
            <div><h2 class="tabStatis">交易份额：</h2></div>
        	<div style="border-bottom: 1px dashed #333;padding-bottom:10px;">
                <h3 class="caption"><?php echo $year . '年' . $month . '月用户微视频交易数统计' ;?></h3>
                <img id="order" />
                <p class="totalFee">总计<?php echo $totalOrder; ?>交易数额</p>
